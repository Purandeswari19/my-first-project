...
def add_goal(goal_text: str, priority: str = "medium", target_date: str = "") -> dict[str, Any]:
    ...
    with db_cursor() as cur:
        cur.execute(
            ...
            RETURNING id
            """,
            (goal_text, priority, parsed_target, datetime.now(timezone.utc)),
        )
...
def create_todo(item_text: str, due_date: str = "") -> dict[str, Any]:
    ...
    with db_cursor() as cur:
        cur.execute(
            ...
            RETURNING id
            """,
            (item_text, due, datetime.now(timezone.utc)),
        )
...
def complete_todo(todo_id: int) -> dict[str, Any]:
    ...
    with db_cursor() as cur:
        cur.execute(
            ...
            (datetime.now(timezone.utc), todo_id),
        )
        rows = cur.rowcount
...
def log_meal(meal_type: str, description: str, calories: int = 0) -> dict[str, Any]:
    ...
    with db_cursor() as cur:
        cur.execute(
            ...
            RETURNING id
            """,
            (meal_type, description, calories or None, datetime.now(timezone.utc)),
        )
...
def save_reminder(reminder_type: str, message: str, reminder_time: str) -> dict[str, Any]:
    ...
    with db_cursor() as cur:
        cur.execute(
            ...
            RETURNING id
            """,
            (reminder_type, message, reminder_time, datetime.now(timezone.utc)),
        )
