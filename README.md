...
```bash
python -m personal_ai_assistant.check_setup
```
This checks Python version, required packages, `.env` values, and PostgreSQL connectivity.
...
- If ADK says `No root_agent found`, make sure you are running from the repository root and selecting app `personal_ai_assistant`.
- If imports fail, run `pip install -r requirements.txt` inside the same Python environment used for `adk web`.
- If PostgreSQL fails, confirm the database exists in pgAdmin and that `DATABASE_URL` has the correct username, password, host, port, and database name.
- If your PostgreSQL password contains special characters such as `@`, `:`, `/`, or `#`, URL-encode it before placing it in `DATABASE_URL`.
